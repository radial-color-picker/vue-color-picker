...
function formatDate(date, formatStr, options = {}) {
    ...
    return formatStr.replace(REGEX_FORMAT, (match, $1) => {
        var _a2, _b;
        return (_b = $1 != null ? $1 : (_a2 = matches[match]) == null ? void 0 : _a2.call(matches)) != null
            ? _b
            : match;
    });
}
    ...
    tasks.reduce((prev, curr) => {
        return prev
            .then((prevRes) => {
                ...
                if (
                    ((_a = result[activeIndex.value]) == null ? void 0 : _a.state) === promiseState.rejected &&
                    interrupt
                ) {
                    onFinished();
                    return;
...
function useDraggable(target, options = {}) {
    ...
    const move = (e) => {
        ...
        if (axis === 'x' || axis === 'both') {
            ...
            if (container) x = Math.min(Math.max(0, x), containerRect.width + container.scrollLeft - targetRect.width);
        }
        if (axis === 'y' || axis === 'both') {
            y = e.clientY - pressedDelta.value.y;
            if (container) y = Math.min(Math.max(0, y), containerRect.height + container.scrollTop - targetRect.height);
        }
        position.value = {
            x,
            y,
